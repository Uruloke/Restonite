...
internal class InstallTypeEditor
{
    ...
    public void Setup(Slot slot, Sync<int> field)
    {
        ...
        _textDrive.Content.Value = $"{(InstallType)_target.Value}";
        ui.Style.FlexibleWidth = -1f;
        ui.Style.MinWidth = 24f;
        text = ">>";
    ...
    private void ShiftEnum(int delta)
    {
        ...
        _target.Value = (int)EnumUtil.ShiftEnum((IConvertible)(InstallType)_target.Value, delta);
        _textDrive.Content.Value = $"{(InstallType)_target.Value}";
    }
